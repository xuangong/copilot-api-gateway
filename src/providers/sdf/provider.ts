...
import type { ModelProvider, ProbeResult, ProviderFetchOptions } from "../types"
...
export class SdfProvider implements ModelProvider {
  ...
  async probe(): Promise<ProbeResult> {
    return probeViaModels(() => this.getModels() as unknown as Promise<{ data?: Array<{ id?: string }> }>)
  }
  ...
  async fetch(endpoint: EndpointKey, init: RequestInit, opts: ProviderFetchOptions = {}): Promise<Response> {
    ...
    const rewrittenBody: BodyInit = bodyIsFormData
      ...
      : (rewriteJsonModel(typeof init.body === "string" ? init.body : JSON.stringify({})) as string)
    ...
    outHeaders.set("X-CV", `root.${randomShort()}`)
    outHeaders.set("X-InteractionId", crypto.randomUUID())
    outHeaders.set("X-ScenarioGUID", SCENARIO_GUID)
    if (!bodyIsFormData) outHeaders.set("Content-Type", "application/json")
    if (opts.extraHeaders) {
      for (const [k, v] of Object.entries(opts.extraHeaders)) outHeaders.set(k, v)
    }
    if (init.headers) {
      new Headers(init.headers).forEach((v, k) => outHeaders.set(k, v))
    }
    ...
    try {
      response = await fetchWithRetry(url, {
        ...
        // Clients retry; avoid extra backoff that would inflate subrequest budget.
        maxRetries: 0,
      })
    } catch (err) {
 ...
 */
function rewriteJsonModel(body: string): string {

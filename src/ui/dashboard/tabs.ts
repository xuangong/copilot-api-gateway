          ...
          <div class="relative" x-data="{ userMenuOpen: false }" @click.outside="userMenuOpen = false">
            ...
            <div x-show="userMenuOpen" x-transition class="absolute right-0 mt-2 w-56 rounded-lg shadow-lg border bg-surface-800 border-white/10 py-1 z-50" style="border-color: var(--border-color); background: var(--surface-800);">
              ...
              <button @click="loadSharedByMe(); mySharingOpen = true; userMenuOpen = false"
                ...
                <svg class="w-4 h-4 inline mr-2 opacity-60" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>
              ...
              <button
                ...
                class="w-full text-left px-4 py-2 text-sm text-themed-dim hover:text-themed hover:bg-surface-700 transition-colors cursor-pointer bg-transparent border-0"
              >
                <svg class="w-4 h-4 inline mr-2 opacity-60" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
                <span x-text="t('dash.changePassword')"></span>
              </button>
    ...
    <template x-if="isAdmin || isUser">
      <div x-show="tab === 'upstream'" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0" x-transition:enter-end="opacity-100">
        ...
        <template x-if="isAdmin">
          ...
          <!-- (modal is rendered at top level via renderUpstreamModal() so it
               escapes both the isAdmin / tab-upstream templates that
               kept Alpine from instantiating the inner DOM, and the
               glass-card stacking context that trapped position:fixed.) -->
        </template>
      </div>
    </template>
  `
...
export function renderUpstreamModal(): string {
  return `
    <!-- Add / Edit Upstream modal — rendered at dashboard root so it's
         outside every <template x-if> wrapper. Alpine doesn't reliably
         clone deeply nested x-if templates' contents, which is why
         placing the modal inside renderUpstreamTab() left
         querySelector('[x-show="upstreamForm.open"]') returning null. -->

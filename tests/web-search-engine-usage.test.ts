...
describe("web_search_engine_usage", () => {
  test("aggregates per-engine attempts with success/failure/empty/duration", async () => {
    ...
    await repo.webSearchEngineUsage.record("k1", "msGrounding", hour, { ok: true, resultCount: 3, durationMs: 400 })
    ...
    expect(ms!.attempts).toBe(2)
    expect(ms!.failures).toBe(1)
    expect(ms!.successes).toBe(1)
    expect(ms!.emptyResults).toBe(0)
    expect(ms!.successDurationMs).toBe(400)
    expect(ms!.failureDurationMs).toBe(250)
    ...
    expect(bing!.successDurationMs).toBe(1400)
    expect(bing!.failureDurationMs).toBe(0)
  })
})

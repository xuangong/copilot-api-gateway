...
import { withInlineImagesCompressed } from "~/providers/copilot/interceptors/chat-completions/with-inline-images-compressed"
...
describe("withInlineImagesCompressed (chat_completions)", () => {
  ...
  test("skips compression when flag disabled but still delegates", async () => {
    ...
    let runCalls = 0
    await withInlineImagesCompressed(inv, ctx, async () => {
      runCalls++
      return FAKE_RESPONSE
    })
  ...
  test("propagates terminal response unchanged", async () => {
    const inv = makeInv(true, true)

/**
 * Auth ephemeral stores — Week 5b port of src/routes/auth/stores.ts.
 *
 * KV-backed (production CFW) or in-memory Map fallback (local Bun dev/tests).
 * Holds short-lived secrets: OAuth state, email verification codes, magic
 * link tokens. All entries are single-use + TTL-bounded.
 */
import {
  OAUTH_STATE_TTL_MS,
  EMAIL_CODE_TTL_MS,
...
export function initOAuthKV(kv: KVNamespace) {
  oauthKV = kv
}

export function resetAuthStoresForTest() {
  oauthKV = null
...
export async function saveOAuthState(
  state: string,
  data: { inviteCode?: string; createdAt: number },
) {
  if (oauthKV) {
    await oauthKV.put(`oauth_state:${state}`, JSON.stringify(data), {
      expirationTtl: 600,
    })
  } else {
    cleanupOAuthStates()
...
export async function getOAuthState(
  state: string,
): Promise<{ inviteCode?: string; createdAt: number } | null> {
  if (oauthKV) {
...
export async function saveEmailCode(
  email: string,
  data: { code: string; inviteCode: string; name: string; password: string },
) {
  const entry = { ...data, createdAt: Date.now() }
  if (oauthKV) {
    await oauthKV.put(`email_code:${email}`, JSON.stringify(entry), {
      expirationTtl: 600,
    })
  } else {
    const now = Date.now()
...
export async function getEmailCode(
  email: string,
): Promise<{ code: string; inviteCode: string; name: string; password: string } | null> {
  if (oauthKV) {
...
// === Magic link token (passwordless login) ===

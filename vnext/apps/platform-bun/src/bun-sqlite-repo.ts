...
function migrateSchema(db: Database): void {
  ...
  // Spec 14: per-key request dump (rolling window in seconds; NULL = disabled).
  if (!hasColumn(db, "api_keys", "dump_retention_seconds")) {
    db.exec("ALTER TABLE api_keys ADD COLUMN dump_retention_seconds INTEGER")
  }
  // spilled_files must be created BEFORE dump_records because the dump_records
  // triggers reference it.
  db.exec(`CREATE TABLE IF NOT EXISTS spilled_files (
  ...
  db.exec(`CREATE UNIQUE INDEX IF NOT EXISTS idx_spilled_files_owned_owner
    ON spilled_files (owner_kind, owner_key)
    WHERE state = 'owned'`)
  db.exec(`CREATE INDEX IF NOT EXISTS idx_spilled_files_collectible
    ON spilled_files (collect_after, file_key)
    WHERE state != 'owned'`)
  db.exec(`CREATE TABLE IF NOT EXISTS dump_records (
    key_id TEXT NOT NULL,
    id TEXT NOT NULL,
  ...
  db.exec(`CREATE INDEX IF NOT EXISTS idx_dump_records_key_created
    ON dump_records(key_id, created_at DESC)`)
  // Triggers adopt/retire request+response spilled files as dump_records
  // rows are inserted/deleted. Each descriptor is a JSON blob whose `key`
  // field carries the file_key.
  db.exec(`CREATE TRIGGER IF NOT EXISTS dump_records_adopt_request_insert
    ...
    END`)
  db.exec(`CREATE TRIGGER IF NOT EXISTS dump_records_adopt_response_insert
    AFTER INSERT ON dump_records
    ...
    BEGIN
      ...
      WHERE file_key = json_extract(NEW.response_body_descriptor, '$.key')
        ...
        AND claim_token IS NULL;
    END`)
  db.exec(`CREATE TRIGGER IF NOT EXISTS dump_records_retire_request_delete
    ...
    BEGIN
      ...
      WHERE file_key = json_extract(OLD.request_body_descriptor, '$.key')
        AND state = 'owned';
    END`)
  db.exec(`CREATE TRIGGER IF NOT EXISTS dump_records_retire_response_delete
    AFTER DELETE ON dump_records

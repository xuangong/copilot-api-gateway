...
export const isDirectFallbackId = (
  id: string,
): id is (typeof DIRECT_FALLBACK_IDS)[number] =>
...
// match against `getRuntimeLocation` (which uppercases CF's `request.cf.colo`
// and the Node `RUNTIME_LOCATION` env var) and the dashboard's free-form
// input stay aligned.
export const normalizeProxyFallbackList = (
...
const normalizeColos = (
  colos: readonly string[] | undefined,
): string[] | undefined => {
...
// empty array and `normalizeProxyFallbackList` strips one before storage.
export const entryMatchesColo = (
  entry: ProxyFallbackEntry,
  currentColo: string,
): boolean => entry.colos === undefined || entry.colos.includes(currentColo)

// Opaque-value trailer codec — ported verbatim from
// copilot-gateway/packages/protocols/src/common/opaque-value.ts.
//
// Some upstream ids (Copilot's Responses stream) are opaque tokens that
// downstream consumers must round-trip exactly. But the gateway needs to
// smuggle its own metadata (e.g. the Copilot-side id when we replace an
// item id with a stable one). This codec framed-appends a trailer to
// an existing opaque value using a 2-byte length marker at the tail, so
// callers can later split the trailer back out even when the original
// value had no delimiter.
//
// Encoding rules:
// - Detect base64 vs base64url canonical encoding of the input; preserve
//   origin on re-encode so the wire form matches.
// - Trailer length is capped at 0xffff (2-byte marker).
// - `raw` origin (neither canonical b64/b64url) round-trips through
//   UTF-16 codepoint pairs so we don't lose non-ASCII bytes.

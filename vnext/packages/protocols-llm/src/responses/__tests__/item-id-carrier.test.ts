...
describe('Copilot item id carrier', () => {
  test.each([
    ...
    ['base64url', '--__'],
    ['empty raw', ''],
  ])('round-trips %s blobs and their upstream ids', (label, original) => {
    ...
    expect(unwrapCopilotItemId(wrapped)).toEqual({
      ...
      value: original,
      version: 1,
      origin: label === 'base64' ? 'base64' : label === 'base64url' ? 'base64url' : 'raw',
      id: 'rs_upstream',
    })
  })
  ...
  test('rejects empty ids', () => {
    expect(() => wrapCopilotItemId('opaque', '')).toThrow(TypeError)
  })
})

...
// Copilot's Responses stream returns opaque upstream ids that clients must
// round-trip back on the next turn. The gateway also needs to smuggle the
// original upstream id inside a stable client-facing id so item-id membrane
// can restore it on subsequent turns. This carrier appends the id + origin
// as a JSON trailer via the opaque-value trailer codec, then re-encodes so
// the carrier is itself a valid opaque value in the same alphabet.
//
// Ported verbatim from
// copilot-gateway/packages/provider-copilot/src/interceptors/responses/item-id-carrier.ts.
...
export const wrapCopilotItemId = (value: string, id: string): string => {
  ...
  if (metadata.length > MAX_OPAQUE_TRAILER_BYTES) {
    throw new RangeError('Copilot item id metadata exceeds the 2-byte length marker')
  }

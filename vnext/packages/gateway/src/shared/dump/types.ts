// Per-API-key request-dump types (ported from copilot-gateway/dump/types.ts).
// Adjusted for vNext:
//   - No `UpstreamColor` — vNext upstreams table has no `color` column, so
//     `DumpUpstreamRef` drops the field entirely.
//   - `UpstreamKind` is imported from `@vibe-llm/protocols-llm/common` (the
//     union `'copilot'|'custom'|'azure'|'sdf'`), which is vNext's equivalent
//     of the reference `UpstreamProviderKind`.
//
// Three shapes split by lifecycle:
...
// `ApiErrorResult.source`) or an uncategorized failure (anything the respond
// layer / passthrough-serve caught or observed mid-flight: thrown exceptions,
// source-emitted error events, downstream cancels, write errors) carrying
// its one-line reason text. The categorized form stores no status —
// `DumpMetadata.status` already does.
...
// same `collectXProtocolEventsToResult` reducer.

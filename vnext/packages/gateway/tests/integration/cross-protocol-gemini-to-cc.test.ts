 ...
 * the gateway resolves the binding for a `/v1beta/models/<model>:generateContent`
 * (or `streamGenerateContent`) request the pair-selector picks
 * `gemini → chat_completions` (third preference after messages → responses)
 * and `traverseTranslation` runs:
 *
 *   1. `translateGeminiToChat` shapes the gemini payload into a chat-completions
 ...
 * `"candidates"` + no `[DONE]` hub terminator) and non-streaming (assert
 * `body.candidates[0].content` is defined). The captured upstream URL must end
 * with `/chat/completions` — that's the load-bearing proof that the cross-
 ...
 * We advertise ONLY chat_completions to force the third-preference binding.
 */
...
const env = {} as never
const MODEL_ID = 'my-llm-gemini-to-cc'

// Custom upstream advertising ONLY chat_completions — forces gemini client
// into the cross-protocol path (gemini → chat_completions) via
// `pair-selector.ts` (preference: gemini → messages → responses → chat_completions).
const customCCUpstream = (): UpstreamRecord => ({
  id: 'up_custom_gemini_to_cc',
  provider: 'custom',
  name: 'my-llm',
  enabled: true,
  sortOrder: 0,
...
function installFetchCapture(): FetchInstall {
  ...
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    // gemini→chat_completions; a regression would land on /messages and fail.
    capturedPath = url.pathname
    if (url.pathname.endsWith('/chat/completions')) {
      // Canned chat-completion JSON envelope. `synthesizeChatFramesFromJson`
      // turns this into hub chat_completions frames which the gemini
      // translator then maps back to gemini wire format.
      const body = {
        id: 'chatcmpl_xprot_gemini_to_cc_1',
...
test('gemini client → chat_completions upstream (non-streaming): returns gemini JSON; upstream URL is /chat/completions', async () => {
  ...
  // be defined. The chat_completions hub response was translated back into
  // gemini wire format by translateChatToGeminiBody via translateBody.
  expect(json.candidates).toBeDefined()
  expect(json.candidates[0]?.content).toBeDefined()

  const path = install.capturedPath()
  expect(path).not.toBeNull()
  // Upstream must have hit /chat/completions, NOT /messages or /responses.
...
test('gemini client → chat_completions upstream (streaming): returns SSE body; upstream URL is /chat/completions', async () => {
  initRepo(stubRepo([customCCUpstream()]))
  initBackground({ waitUntil: (p) => { void p.catch(() => {}) } })
  initRuntimeLocation('bun')
  const install = installFetchCapture()
  ...
  // which runs translateChatToGeminiSSE at SSE-time. The translated output
  // yields gemini-shaped events, so the SSE body must contain "candidates"
  // (the gemini frame marker), NOT "[DONE]" (the chat_completions hub terminator).

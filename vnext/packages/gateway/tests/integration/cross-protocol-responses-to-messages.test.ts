/**
 * Spec 6 Part 3 Task 6 — cross-protocol integration test: responses client →
 * messages hub.
 *
 * The custom upstream advertises ONLY the `messages` endpoint, so when the
 * gateway resolves the binding for a `/v1/responses` request the pair-selector
 * picks `responses → messages` (first fallback) and `traverseTranslation` runs:
 *
 *   1. `translateResponsesToMessages` shapes the responses payload into a
 *      messages payload the upstream understands;
 *   2. The hub messages attempt fetches `<baseUrl>/messages`, gets a JSON
 *      envelope back (content-type: application/json), and
 *      `synthesizeMessagesFramesFromJson` produces hub frames;
 *   3. `translateMessagesToResponsesSSE` (streaming) or
 *      `translateMessagesToResponsesBody` (non-streaming, via
 *      `EventResult.translateBody`) maps the hub-shaped output back into
 *      responses wire format.
 *
 * Both branches are asserted: streaming (assert `data:` chunks containing
 * `response.completed`) and non-streaming (assert `object: 'response'`,
 * `output[0].role === 'assistant'`). The captured upstream URL must end with
 * `/messages` — that's the load-bearing proof that the cross-protocol path
 * was actually exercised.
 */
import { test, expect, afterEach } from 'bun:test'
import { Hono } from 'hono'
import { app as innerApp } from '../../src/app.ts'
import { initRepo } from '../../src/shared/repo/index.ts'
import { initResponsesStore } from '../../src/shared/runtime/responses-store.ts'
import { initBackground, initRuntimeLocation, __resetPlatformForTests } from '@vnext/platform'
import { InMemoryResponsesSnapshotStore } from '@vnext/responses-store'
import type { Repo, UpstreamRecord } from '../../src/shared/repo/types.ts'
import type { DataPlaneAuthCtx } from '../../src/data-plane/models/routes.ts'

const env = {} as never
const MODEL_ID = 'my-llm-resp-to-msg'

// Custom upstream advertising ONLY messages — forces responses client into
// the cross-protocol path (responses → messages) via `pair-selector.ts`
// (preference: responses → messages → chat_completions).
const customMessagesUpstream = (): UpstreamRecord => ({
  id: 'up_custom_resp_to_msg',
  provider: 'custom',
  name: 'my-llm',
  enabled: true,
  sortOrder: 0,
  config: {
    name: 'my-llm',
    baseUrl: 'https://api.example.com/v1',
    apiKey: 'sk-secret',
    endpoints: ['messages'],
  },
  flagOverrides: {},
  disabledPublicModelIds: [],
  createdAt: '2026-01-01T00:00:00Z',
  updatedAt: '2026-01-01T00:00:00Z',
})

const stubRepo = (upstreams: UpstreamRecord[]): Repo => ({
  upstreams: { list: async () => upstreams },
} as unknown as Repo)

const originalFetch = globalThis.fetch

interface FetchInstall {
  /** URL pathname of the most recent non-/models POST. */
  capturedPath: () => string | null
}

/**
 * Override globalThis.fetch with a handler that:
 *   - returns an OpenAI-shaped `/models` list (single model) for GETs to
 *     `<baseUrl>/models`,
 *   - returns a canned `MessagesResult` JSON envelope (content-type:
 *     application/json) for POSTs to `<baseUrl>/messages`. The messages
 *     attempt's JSON branch covers BOTH client modes when content-type
 *     signals JSON, so a single canned response works for both stream:true
 *     and stream:false at the client side.
 */
function installFetchCapture(): FetchInstall {
  let capturedPath: string | null = null
  globalThis.fetch = (async (input: RequestInfo | URL, init?: RequestInit) => {
    const req = input instanceof Request ? input : new Request(input as string, init)
    const url = new URL(req.url)
    if (url.pathname.endsWith('/models')) {
      return new Response(
        JSON.stringify({
          object: 'list',
          data: [{ id: MODEL_ID, object: 'model', owned_by: 'my-llm' }],
        }),
        { status: 200, headers: { 'content-type': 'application/json' } },
      )
    }
    // Capture the pathname so the test can assert which upstream endpoint was
    // actually hit. We expect /messages for cross-protocol responses→messages;
    // a regression would land on /responses and fail.
    capturedPath = url.pathname
    if (url.pathname.endsWith('/messages')) {
      // Canned MessagesResult envelope. `synthesizeMessagesFramesFromJson`
      // emits `message_start` → `content_block_*` → `message_delta` →
      // `message_stop` from this shape, which is exactly what the responses
      // translator needs to produce a `response` envelope with the content.
      const body = {
        id: 'msg_xprot_resp_to_msg_1',
        type: 'message',
        role: 'assistant',
        model: MODEL_ID,
        content: [{ type: 'text', text: 'ok' }],
        stop_reason: 'end_turn',
        stop_sequence: null,
        usage: { input_tokens: 3, output_tokens: 5 },
      }
      return new Response(JSON.stringify(body), {
        status: 200,
        headers: { 'content-type': 'application/json' },
      })
    }
    return new Response('not found', { status: 404 })
  }) as typeof fetch
  return { capturedPath: () => capturedPath }
}

afterEach(() => {
  globalThis.fetch = originalFetch
  __resetPlatformForTests()
})

function buildApp(auth: DataPlaneAuthCtx) {
  const wrapper = new Hono()
  wrapper.use('*', (c, next) => { c.set('auth', auth); return next() })
  wrapper.route('/', innerApp)
  return wrapper
}

async function postResponses(body: Record<string, unknown>): Promise<Response> {
  const app = buildApp({})
  const req = new Request('http://local/v1/responses', {
    method: 'POST',
    headers: { 'content-type': 'application/json' },
    body: JSON.stringify(body),
  })
  return app.fetch(req, env)
}

async function readSSEText(res: Response): Promise<string> {
  const reader = res.body?.getReader()
  if (!reader) return ''
  const decoder = new TextDecoder()
  let out = ''
  for (;;) {
    const { value, done } = await reader.read()
    if (done) return out
    if (value) out += decoder.decode(value, { stream: true })
  }
}

test('responses client → messages upstream (non-streaming): returns responses JSON; upstream URL is /messages', async () => {
  initRepo(stubRepo([customMessagesUpstream()]))
  initResponsesStore(new InMemoryResponsesSnapshotStore())
  initBackground({ waitUntil: (p) => { void p.catch(() => {}) } })
  initRuntimeLocation('bun')
  const install = installFetchCapture()

  const res = await postResponses({
    model: MODEL_ID,
    stream: false,
    input: [{ type: 'message', role: 'user', content: 'hi' }],
  })
  expect(res.status).toBe(200)

  const json = await res.json() as {
    object: string
    output: Array<{ type: string; role: string; content: Array<{ type: string; text: string }> }>
  }
  expect(json.object).toBe('response')
  expect(json.output[0]?.role).toBe('assistant')

  const path = install.capturedPath()
  expect(path).not.toBeNull()
  // Cross-protocol proof: upstream must have hit /messages, NOT /responses.
  // A regression that bypasses traverseTranslation would either 404 (no
  // responses endpoint) or accidentally route through identity — both fail
  // this assertion.
  expect(path!.endsWith('/messages')).toBe(true)
  expect(path!.endsWith('/responses')).toBe(false)
})

test('responses client → messages upstream (streaming): returns SSE body; upstream URL is /messages', async () => {
  initRepo(stubRepo([customMessagesUpstream()]))
  initResponsesStore(new InMemoryResponsesSnapshotStore())
  initBackground({ waitUntil: (p) => { void p.catch(() => {}) } })
  initRuntimeLocation('bun')
  const install = installFetchCapture()

  const res = await postResponses({
    model: MODEL_ID,
    stream: true,
    input: [{ type: 'message', role: 'user', content: 'hi' }],
  })
  expect(res.status).toBe(200)
  expect(res.headers.get('content-type') ?? '').toContain('text/event-stream')

  const body = await readSSEText(res)
  // After fix: responses/respond.ts streaming branch applies
  // applyTranslatorEventsForStreaming which runs translateMessagesToResponsesSSE
  // at SSE-time. The translated output yields responses-shaped events, so the
  // SSE body must contain "response.completed" (the responses protocol terminator),
  // NOT "message_stop" (the messages hub terminator).
  expect(body).toContain('data:')
  expect(body).toContain('"type":"response.completed"')
  expect(body).not.toContain('message_stop')

  const path = install.capturedPath()
  expect(path).not.toBeNull()
  expect(path!.endsWith('/messages')).toBe(true)
  expect(path!.endsWith('/responses')).toBe(false)
})

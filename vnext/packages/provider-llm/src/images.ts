/**
 * Ported 1:1 from copilot-gateway packages/provider/src/images.ts.
 *
 * The `image_generation` responses server-tool shim constructs an
 * `ImagesEditsRequest` at dispatch time; the shim then calls
 * `provider.fetch({ endpoint: 'images_edits', payload })` with either the
 * multipart FormData or JSON body produced by
 * `serializeOpenAIImagesEditsRequest`. vNext providers (azure, custom) already
 * accept both body types via their generic `fetch(ProviderRequest)` surface,
 * so no per-provider changes are required.
 */
import { base64ToBytes, bytesToBase64, parseBase64ImageDataUrl } from './image-helpers.ts'
import type { ImageEditReference } from '@vibe-llm/protocols/images'

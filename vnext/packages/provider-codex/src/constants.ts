...
// to the official Codex CLI shape. Do not add gateway/operator attribution as
// a product token or trailing User-Agent suffix.
...
// OAuth User-Agent. Pinned independently of CODEX_CLI_VERSION: `0.91.0` was
// captured by OpenAI when the codex-cli OAuth client was first registered,
// and auth.openai.com continues to accept it across CLI revisions. Note the
// hyphen-lowercase product name — distinct from the underscore form used on
// the data plane below.
...
// Native unary compaction endpoint. Codex CLI defaults to a client-side
// `RemoteCompactionV2` path that re-uses `/codex/responses` with a compaction
// trigger item, but the server still serves this canonical
// `/responses/compact` URL — the same one Azure OpenAI and the public
// `api.openai.com` Responses surface expose. Prefer the unary endpoint so
// the provider skips the SSE drain entirely.
...
// latest stable release at https://github.com/openai/codex/releases — newer
// entries in /codex/models gate themselves behind a `minimal_client_version`
// (e.g. gpt-5.6 Sol / Terra / Luna family needs 0.144.0+), so a stale value
...
// OS/arch/terminal segment; do not append gateway/operator/MCP attribution.

// Codex /codex/models catalog fetch + shape adapter.
//
// Ported from copilot-gateway/packages/provider-codex/src/models.ts.
//
// vNext adaptations:
//   - `Fetcher` from local `./fetcher` (not the reference project's
//     `@floway-dev/provider` boundary).
//   - Reference project's `ProviderModel` / `UpstreamChatModelConfig` /
//     `FlagId` were framework-owned types; vNext has no equivalent yet.
//     Return a plain object shape compatible with
//     `ProviderModelsResponse.data` (which is `any[]` at the framework
//     boundary). Dashboard/UI adapters can pick fields as needed.
//   - `enabledFlags` parameter dropped — the resolved flag set lives on the
//     upstream record at request time; per-model propagation can be added
//     back if a downstream consumer needs it.
...
export interface CodexRawModel {
  ...
  // vNext has no override path, so only the operational value is kept.
...
export const fetchCodexCatalog = async (opts: {
  ...
  signal?: AbortSignal
  fetcher: Fetcher
}): Promise<CodexRawModel[]> => {
  const response = await opts.fetcher(
    `${CODEX_BACKEND_BASE}${CODEX_MODELS_PATH}?client_version=${CODEX_CLI_VERSION}`,
    {
      method: 'GET',
      headers: {
...
// are tolerated when absent for backwards compatibility with older catalog
// snapshots, but throw on type drift.
const assertRawModel = (value: unknown): CodexRawModel => {
  ...
  if (typeof display_name !== 'string') {
    throw new TypeError(`Codex model entry ${slug} missing display_name`)
  }
  const context_window = value.context_window
  if (typeof context_window !== 'number') {
    throw new TypeError(`Codex model entry ${slug} missing context_window`)
  }
  ...
  if (value.input_modalities !== undefined) {
    if (!Array.isArray(value.input_modalities)) {
      throw new TypeError(`Codex model entry ${slug} input_modalities not an array`)
    }
    const out: ('text' | 'image')[] = []
    for (const m of value.input_modalities) {
      if (m !== 'text' && m !== 'image') {
        throw new TypeError(`Codex model entry ${slug} unknown modality ${JSON.stringify(m)}`)
      }
  ...
  if (value.supported_reasoning_levels !== undefined) {
    if (!Array.isArray(value.supported_reasoning_levels)) {
      throw new TypeError(`Codex model entry ${slug} supported_reasoning_levels not an array`)
    }
  ...
  if (raw.reasoning_efforts && raw.reasoning_efforts.length > 0) {
    ...
    if (raw.default_reasoning_effort !== undefined) {
      if (!raw.reasoning_efforts.includes(raw.default_reasoning_effort)) {
        throw new Error(
          `Codex model ${raw.id}: default_reasoning_level not in supported_reasoning_levels`,
        )
      }
      effortDefault = raw.default_reasoning_effort
    } else {
      effortDefault = raw.reasoning_efforts.includes('medium')
        ? 'medium'
        : raw.reasoning_efforts[0]!
    }
